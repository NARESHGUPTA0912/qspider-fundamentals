...
* It has some extra features such as: 1) Event Handling 2) AOP Integration 3) Easier Configuration
  1\) It supports both xml and annotation configuration.
...
```
<!-- 9 mar -->
-- Spring Code Example only
- Important Points(Summary):
-> If we want to check tomcat server is open or not so we just search on browser: localhost:8080 . It shows the tomcat page.
...
```
- Important Points:
-> Afterthat generate EmployeeDto with getters and setters: We are using for t/f data from controller -> service -> DAO -> Controller. store json object in dto class
-> @RequestBody used json convert to java (we have to annotate the save method for saving JSON object in Java, if we did not annotate then java variables did not stored json and it returns null).
-> Give port no in application.properties
-> JSON to Java known as Serialization and Java to JSON Known as Deserialization.
-> copy Spring Devtool dependency from spring initializer website so everytime we changes on code then server restart automatically
-> @PathVariable used when pass data through url
-> ResponseEntity<T>(C) used to show response while fetching. It have one ResponseEntity.ok(). which used as status code.
->create 4 methods : todayDate, saveEmp, saveAllEmp, getEmpbyid, 
-> Cross horizon (we have instead of Cors in springboot).
-> SwaggerUI works same as Postman API . copy dependency from maven dependency also used for creating documentation for frontend dev.(gives by Open API)
...
- Important Points:
-> copy Spring Data JPA(Hibernate+JPA) and mysql driver dependency from Spring initializer then paste and update project.
-> give db(spring.jpa.hibernate) details in application.properties (we can write from the internet).
-> First generate Entity class with getters and setters .
-> create Repository(I) which extends the JPARepository<T,ID>(I) and annotate with @Repository in Repository package, it gives multiple features for crud operations, pagination, sorting etc.
-> Afterthat we create Service(C) annotate with @Service for pass the data and here we create all service methods.
-> EmployeeDto used for JSON validation(controller -> Service||Service -> Controller) and Employee entity used for save data(used for communicate with db).
-> data comes from db to store in Employee(entity) and then t/f thrugh DTO which sends data between client to server and server to client and service to controller and controller to srvice.[client -> DTO -> Employee -> DTO -> Client] :: we do not show the password(ignore using @JsonIgnore) after save.
-> if we sent the similar data again then bts only update query run if primary key same.(if every value same then it did not update, insert anything).
-> create utility class [RequestResponse(C)] for convert data jsontojava and javatojson.
-> create saveAllEmp() method in List<Employee>  then iterate using for-each.
-> use @RequestBody of SwaggerUI insted of Spring Data JPA.
-> create simple method of deleteEmpById, getEmpById.
...
-> we have to create dto in between and applying validation .
...
------------------------------------------------------------------------------------------------------------------------
# Some other Classes and Interfaces:
1) ResponseEntity(C): 
2) JpaRepository< , > (I) : save(entity)
                            findAll()
                            findById(id)
                            deleteById(id)
